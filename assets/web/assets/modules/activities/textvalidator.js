...
class TextValidator {
  ...
  async initialize() {
    try {
      ...
      console.log('Fallback dictionary initialized');
      ...
      try {
        ...
        console.log(`Extended dictionary loaded with ${this.spanishWords.size} words total`);
      } catch (dictError) {
  ...
  initializeFallbackDictionary() {
    ...
    console.log(`Fallback dictionary initialized with ${this.spanishWords.size} common words`);
    this.initialized = true;
  }
  ...
  async isValidText(text) {
    try {
      console.log('Starting text validation for:', text);
      ...
      // Skip empty text
      if (!cleanText) {
        console.log('Text is empty, validation failed');
  ...
  if (word.length > 3) {
    ...
    for (const verbForm of verbEndings) {
      if (word.length > verbForm.ending.length + 2 && word.endsWith(verbForm.ending)) {
        ...
        console.log(`Verb conjugation: ${word} → ${infinitive}`);
      }
    }
  }
    ...
    for (const variation of genderVariations) {
      if (word.length > variation.ending.length + 2 && word.endsWith(variation.ending)) {
        ...
        stems.push(newStem);
        console.log(`Gender variation: ${word} → ${newStem}`);
      }
    }
    ...
    // Add logging to see what stems we're generating
    console.log(`Generated stems for "${word}":`, uniqueStems);
