    ...
    def test_pipeline_integration_first_five_pages(self):
        ...
        with tempfile.TemporaryDirectory() as self.temp_dir:
            ...
            self.assertFileCount(
                ...
                5,
                "Unexpected number of cropped images created",
            )
            self.assertFileCount(
                "images/img_p*_r*_recrop.png",
                5,
                "Unexpected number of recropped images created",
            )
            self.assertFileCount(
                "images/img_p*_chart.png",
                10,
            ...
            self.assertFileContains(
                "metadata_report.html",
                "Hyena and Raven",
                "Hyena and Raven title not found in metadata report",
            )
            self.assertFileContains(
                "metadata_report.html",
                "Tony Lelliott",
                "Tony Lelliott not found as an author in metadata report",
            )

            self.assertFileContainsIgnoreCase(
                "page_report.html",
                "corbeau",
