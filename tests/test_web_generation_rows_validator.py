...
from adt_press.llm.web_generation_rows import GenerationResponse, Row, Column
...
class TestRowsDataIdValidator:
    ...
    def test_valid_rows_with_text_and_image_ids(self):
        ...
        rows = [
            Row(columns=[
                Column(span=6, parts=["text-1", "image-1"]),
                Column(span=6, parts=["text-2"])
            ]),
            Row(columns=[
                Column(span=12, parts=["image-2", "text-3"])
            ])
        ]
    ...
    def test_invalid_part_id_raises_error(self):
        ...
        rows = [
            Row(columns=[
                Column(span=6, parts=["invalid-id"]),
                Column(span=6, parts=["text-1"])
            ])
        ]
