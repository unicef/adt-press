...
from adt_press.utils.labels import clean_label_value, slug_from_pdf_path
...
def label_config(config: DictConfig, pdf_path_config: str) -> str:
    ...
    log.info("label derived from pdf", label=generated_label, source=os.path.splitext(os.path.basename(pdf_path_config))[0])
