...
def pages_for_pdf(output_dir: str, pdf_path: str, start_page: int, end_page: int) -> list[Page]:
    ...
    # Convert to our models and adjust paths
    pages = []
    for page_data in extract_data["pages"]:
        # Convert images and adjust paths to be relative to main output_dir
        images = []
        for img_data in page_data["images"]:
            image = Image(
                ...
                image_path=os.path.join(extract_dir, img_data["image_path"]),
                chart_path=os.path.join(extract_dir, img_data["chart_path"]),
        ...
        # Create page object with adjusted path
        page = Page(
            page_id=page_data["page_id"],
            page_number=page_data["page_number"],
            page_image_path=os.path.join(extract_dir, page_data["page_image_path"]),
            text=page_data["text"],
            images=images
        )
        pages.append(page)
